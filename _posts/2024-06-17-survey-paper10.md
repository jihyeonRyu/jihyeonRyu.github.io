...
- 종류
  - Sparse Attention
    ...
    - 주요 접근 방식
      - Longformer, ETC, BIGBIRD: 로컬 윈도우 기반 어텐션와 task-specific 글로벌 어텐션을 결합하여 설프 어텐션 복잡도를 선형으로 줄입니다.
      - HEPOS: 각 어텐션 헤드가 입력 시퀀스의 특정 부분에 집중하도록 하여 효율적인 인코더-디코더 처리를 가능하게 합니다.
      - MATE: 주의를 다중 뷰 형식으로 변환하여 테이블의 행 또는 열을 효율적으로 처리합니다.
      - TDANet: 인간 뇌의 상위-하위 주의 메커니즘을 모방하여 가장 관련성 높은 정보에 선택적으로 집중합니다.
      - ALBERT: 계층 간 파라미터 공유를 통해 파라미터 수를 89% 줄이면서도 정확성을 보장합니다.
  - Approximate Attention
    - 자가 주의 행렬의 저랭크 근사 및 자가 주의 메커니즘의 혁신적인 재구성을 포함합니다. 이러한 접근 방식은 N × N 행렬의 직접적인 계산을 피하여 계산 복잡도를 줄이고 효율성을 높입니다.
    - 주요 접근 방식:
      - Linformer: 주의 행렬을 저랭크 행렬로 효과적으로 분해하여 메모리 복잡도를 크게 줄입니다.
      - Reformer: 기존의 dot-product 주의를 locality-sensitive hashing으로 대체합니다.
      - Katharopoulos et al.: 매트릭스 곱셈의 연관성을 활용하여 자가 주의 가중치를 계산하는 커널 기반 접근 방식을 도입합니다.
      - Polysketchforme: 다항 함수와 스케치 기술을 사용하여 소프트맥스 주의 출력을 근사화합니다.
      - Mega: 단일 헤드 게이트 주의 메커니즘에 지수 이동 평균을 통합하여 위치 의존적인 지역 종속성을 포함합니다.
      - Deformable Attention: 데이터 인식 변형 주의 메커니즘을 제안하여 전통적인 밀집 주의 방식에 비해 성능을 개선합니다.
      - CrossViT: 변동 크기의 입력 토큰을 효율적으로 처리하면서 계산 비용을 줄이는 선형 크로스 주의 메커니즘을 도입합니다.
  - Attention-Free Approaches
    ...
    - 주요 접근 방식:
      ...
      - SSMs: 주의 메커니즘을 상태 공간 모델(SSM)로 대체하는 접근법.
      ...
      - RetNet: 다중 헤드 주의 메커니즘을 다중 스케일 유지 메커니즘(multi-scale retention mechanism)으로 대체한 아키텍처.